...
def front_matter(title: str, description: str, weight: int, source: str) -> str:
    # The page is only ever a copy, so it says so where anyone opening it
    # looks first — an edit here is lost on the next build.
    return (
        "---\n"
        f"# Generated by site/sync-docs.py from {source}. Do not edit:\n"
        f"# the next build overwrites it. Edit {source} instead.\n"
        f'title: "{title}"\n'
